#!/usr/bin/python2.7
# Compresses the core SparqlBlocks files into a single JavaScript file.
#
...
# This script generates two versions of SparqlBlocks's core files:
#   sparqlblocks_compressed.js
#   sparqlblocks_uncompressed.js
# The compressed file is a concatenation of all of SparqlBlocks's core files which
# have been run through Google's Closure Compiler.  This is done using the
# online API (which takes a few seconds and requires an Internet connection).
# The uncompressed file is a script that loads in each of SparqlBlocks's core files
...
#   blocks_compressed.js: The compressed SparqlBlocks language blocks.
...
if sys.version_info[0] != 2:
  raise Exception("SparqlBlocks build only compatible with Python 2.x.\n"
...
class Gen_uncompressed(threading.Thread):
  """Generate a JavaScript file that loads SparqlBlocks's raw files.
  ...
  def run(self):
    target_filename = "sparqlblocks_uncompressed.js"
    f = open(target_filename, "w")
    f.write(HEADER)
    f.write("""
// 'this' is 'window' in a browser, or 'global' in node.js.
this.SPARQLBLOCKS_DIR = (function() {
  // Find name of current directory.
  var scripts = document.getElementsByTagName('script');
  var re = new RegExp('(.+)[\/]sparqlblocks_uncompressed\.js$');
  ...
  alert('Could not detect SparqlBlocks\\'s directory name.');
  return '';
})();

this.SPARQLBLOCKS_BOOT = function() {
...
var dir = this.SPARQLBLOCKS_DIR.match(/[^\\/]+$/)[0];
""")
    add_dependency = []
    ...
    add_dependency = "\n".join(add_dependency)
    # Find the SparqlBlocks directory name and replace it with a JS variable.
    # This allows sparqlblocks_uncompressed.js to be compiled on one computer and be
    # used on another, even if the directory name differs.
    m = re.search("[\\/]([^\\/]+)[\\/]core[\\/]sparqlblocks.js", add_dependency)
    ...
    provides.sort()
    f.write("\n")
    f.write("// Load SparqlBlocks.\n")
    for provide in provides:
      f.write("goog.require('%s');\n" % provide)

    f.write("""
delete this.SPARQLBLOCKS_DIR;
delete this.SPARQLBLOCKS_BOOT;
};
...
document.write('<script src="' + this.SPARQLBLOCKS_DIR +
    '/../closure-library/closure/goog/base.js"></script>');
document.write('<script>this.SPARQLBLOCKS_BOOT()</script>');
""")
    f.close()
...
class Gen_compressed(threading.Thread):
  """Generate a JavaScript file that contains all of SparqlBlocks's core and all
  ...
  def run(self):
    self.gen_core()
    self.gen_blocks()
    self.gen_generator("sparql")

  def gen_core(self):
    target_filename = "sparqlblocks_compressed.js"
    ...
    filenames = calcdeps.CalculateDependencies(self.search_paths,
        [os.path.join("core", "sparqlblocks.js")])
  ...
  def gen_blocks(self):
    target_filename = "sparqlblocks_blocks_compressed.js"
    # Define the parameters for the POST request.
    params = [
        ("compilation_level", "SIMPLE_OPTIMIZATIONS"),
        ("output_format", "json"),
    ...
    # Read in all the source files.
    # Add SparqlBlocks.Blocks to be compatible with the compiler.
    params.append(("js_code", "goog.provide('SparqlBlocks.Blocks');"))
    ...
    # Remove SparqlBlocks.Blocks to be compatible with SparqlBlocks.
    remove = "var SparqlBlocks={Blocks:{}};"
  ...
  def gen_generator(self, language):
    ...
    # Read in all the source files.
    # Add SparqlBlocks.Generator to be compatible with the compiler.
    params.append(("js_code", "goog.provide('SparqlBlocks.Generator');"))
    filenames = glob.glob(
    ...
    # Remove SparqlBlocks.Generator to be compatible with SparqlBlocks.
    remove = "var SparqlBlocks={Generator:{}};"
    self.do_compile(params, target_filename, filenames, remove)
