...
# Generate .js files defining SparqlBlocks core and language messages.
#
...
def main():
  ...
  synonym_text = '\n'.join(['SparqlBlocks.Msg.{0} = SparqlBlocks.Msg.{1};'.format(
...
goog.provide('SparqlBlocks.Msg.{0}');
...
""".format(target_lang.replace('-', '.')))
        ...
        for key in sorted_keys:
          ...
          value = value.replace('"', '\\"')
          outfile.write(u'SparqlBlocks.Msg.{0} = "{1}";{2}\n'.format(
              key, value, comment))
