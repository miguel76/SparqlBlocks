...
*/
SparqlBlocks.Output={};
var blockFromStringLiteral_=function(a,b){var c=null,d=a["xml:lang"];d?(c=Blockly.Block.obtain(b,"sparql_text_with_lang"),c.initSvg(),c.setFieldValue(d,"LANG")):(c=Blockly.Block.obtain(b,"sparql_text"),c.initSvg());c.setFieldValue(a.value,"TEXT");return c},blockFromLiteral_=function(a,b){return a.datatype?null:blockFromStringLiteral_(a,b)},blockFromUri_=function(a,b){var c=a.value,d=SparqlBlocks.Prefixes.lookForIri(c);if(null!=d)return c=Blockly.Block.obtain(b,"sparql_prefixed_iri"),c.initSvg(),c.setFieldValue(d.prefix,
"PREFIX"),c.setFieldValue(d.localPart,"LOCAL_NAME"),c;d=Blockly.Block.obtain(b,"sparql_iri");d.initSvg();d.setFieldValue(c,"IRI");return d},blockFromValue_=function(a,b,c){var d=null;if(a)switch(a.type){case "literal":d=blockFromLiteral_(a,b);break;case "uri":d=blockFromUri_(a,b)}d&&(d.setDeletable(!1),d.setMovable(!1),d.setEditable(!1),SparqlBlocks.SelfDuplication.bindMouseEvents(d),c.connect(d.outputConnection),d.render())},blocksFromHeaderVar_=function(a,b,c){a=Blockly.Block.obtain(b,"sparql_column");
a.initSvg();c.connect(a.outputConnection);a.render();return a},containerBlockFromValue_=function(a,b,c){var d=Blockly.Block.obtain(b,"sparql_value_container");d.initSvg();c.connect(d.previousConnection);d.render();c=d.getInput("VALUE").connection;blockFromValue_(a,b,c);return d},colBlockFromVar_=function(a,b,c,d){var e=Blockly.Block.obtain(c,"sparql_column");e.initSvg();e.setFieldValue(a,"COLNAME");d.connect(e.outputConnection);e.render();var f=e.getInput("VALUES").connection;b.forEach(function(a){f=
containerBlockFromValue_(a,c,f).nextConnection});return e},blocksFromSelectResults_=function(a,b){console.log("data: "+JSON.stringify(b));var c=Blockly.Block.obtain(a,"sparql_table");c.initSvg();for(var d=b.head.vars,e=b.results.bindings,f=d.length,g=0;g<f;g++){c.appendValueInput("COL"+(g+1)).setCheck("Column");var h=c.getInput("COL"+(g+1)).connection,l=d[g];colBlockFromVar_(l,e.map(function(a){return a[l]}),a,h)}return c};SparqlBlocks.Output.blocksFromSelectResults=blocksFromSelectResults_;
var fillTableFromSelectResults_=function(a,b){var c=b.head.vars,d=getTableHeaders(c);a.append(d);d=b.results.bindings;for(rowIdx in d)a.append(getTableRow(c,d[rowIdx]))};/*
...
b+e};/*
 ...
 limitations under the License.
*/
SparqlBlocks.Exec={};var defaultEndpointUrl_="http://live.dbpedia.org/sparql",sparqlExec_=function(a,b,c){a||(a=defaultEndpointUrl_);a=a+"?query="+encodeURIComponent(b);return $.ajax({dataType:"json",url:a,success:function(a){c(null,a)},error:function(a,b,f){c({jqXHR:a,textStatus:b,errorThrown:f})}})},sparqlExecAndAlert_=function(a,b){return sparqlExec_(a,b,function(a,b){a?alert("Error: "+a):alert("Results:\n"+JSON.stringify(b))})};SparqlBlocks.Exec.sparqlExecAndAlert=sparqlExecAndAlert_;
var sparqlExecAndBlock_=function(a,b,c,d){},connect_=function(a,b){var c=a.targetBlock();c&&c.dispose();a.connect(b.outputConnection);b.render()},DESCR_LENGTH=40,sparqlExecAndPublish_=function(a,b,c,d,e){var f=Blockly.Block.obtain(c,"sparql_execution_in_progress");f.initSvg();connect_(d,f);return sparqlExec_(a,b,function(a,b){var f=null;if(a){f=Blockly.Block.obtain(c,"sparql_execution_error");f.initSvg();var k=a.textStatus?a.textStatus:"unknown problem";a.jqXHR.status&&(k+=" "+a.jqXHR.status);a.jqXHR.statusText&&
(k+=": "+a.jqXHR.statusText);f.setFieldValue(k,"ERRORTYPE");if(k=a.jqXHR.responseText){var m=null,m=k.length>DESCR_LENGTH?k.substr(0,DESCR_LENGTH-3)+"...":k;f.setFieldValue(m,"ERRORDESCR");f.setTooltip(k)}}else f=SparqlBlocks.Output.blocksFromSelectResults(c,b);connect_(d,f);e()})};SparqlBlocks.Exec.sparqlExecAndPublish=sparqlExecAndPublish_;
SparqlBlocks.Exec.blockExec=function(a){var b=a.getInput("RESULTS");if(b&&(b=b.connection)){var c=SparqlBlocks.Sparql.valueToCode(a,"QUERY",SparqlBlocks.Sparql.ORDER_NONE);if(!c&&!a.sparqlQueryStr||c!=a.sparqlQueryStr)a.sparqlQueryStr=c,a.queryReq&&a.queryReq.abort(),c?(console.log("Ready to execute query: "+c),a.queryReq=SparqlBlocks.Exec.sparqlExecAndPublish(null,c,a.workspace,b,function(){a.queryReq=null})):(console.log("Empty query"),unconnect_(b),a.queryReq=null)}};/*
...
SparqlBlocks.Types.setType=setType;SparqlBlocks.Types.getExtension=getExtension;/*
...
0;c<this.prevBlocks_.length;c++){var d=this.prevBlocks_[c];if(d===a||d===b)d.setParent(null),d.bumpNeighbours_()}this.prevBlocks_[0]=a;this.prevBlocks_[1]=b}}};
